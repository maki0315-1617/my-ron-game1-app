...
export default function Page() {
  ...
  // 位置情報（文字列としてそのままStyleに渡せるようにパーセント表記で管理）
  ...
  const handleAttack = (e) => {
    ...
    // 表示位置の更新（文字列結合によるエラーを避けるためここで反映）
    ...
    setCurrentActionMessage(result + ' 次は守備の番です。ロン君のシュートを止めましょう！');
  ...
  const handleDefend = () => {
    ...
    if (!isOver) {
      setCurrentActionMessage(result + ' ' + (nextRound + 1) + '回戦に突入！あなたの攻撃です。');
      setGameState('attack');
    }
  };
